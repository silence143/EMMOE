...
# @baseline_registry.register_trainer(name="ppo-ctl")
class PPOTrainerCTL(BaseTrainer):
    r"""Basic PPO Trainer."""
    ...
    def __init__(self, config: Config, env):
        ...
        self.images = {'head':[], 'third':[]}
        # self.pre_state_info = state_info
    ...
    def _eval_checkpoint(
        ...
        self.obs_space['robot_arm_depth'] = spaces.Box(shape=(128,128,1), low=0.0, high=1.0, dtype=np.float32)
        self.obs_space['robot_arm_rgb'] = spaces.Box(shape=(128,128,3), low=0, high=255, dtype=np.uint8)
        self.obs_space['robot_head_depth'] = spaces.Box(shape=(128,128,1), low=0.0, high=1.0, dtype=np.float32)
        self.obs_space['robot_head_rgb'] = spaces.Box(shape=(128,128,3), low=0, high=255, dtype=np.uint8)
        ...
        # logger.info('start:',env.env._env._sim.get_state()["robot_state"])
        # Initialize policy inputs
        obs = env.reset()

        self.images['head'].append(obs['robot_head_rgb_256'])
        self.images['third'].append(obs['robot_third_rgb'])


        def resize_array(array, size, mode = 'RGB'):
            if mode == 'L':
                array_L = (array[:,:,0]*255).astype(np.uint8)
                image_L = Image.fromarray(array_L, mode='L')
                resized_image_L = image_L.resize(size)
                resized_array_L = np.array(resized_image_L)/255
                return resized_array_L[:,:,np.newaxis].astype(np.float32)
        ...
        # new_obs = obs.copy()
        # new_obs['robot_arm_depth'] = resize_array(obs['robot_arm_depth'], (128,128), 'L')
        # new_obs['robot_arm_rgb'] = resize_array(obs['robot_arm_rgb'], (128,128))
        # new_obs['robot_head_depth'] = resize_array(obs['robot_head_depth'], (128,128), 'L')
        # new_obs['robot_head_rgb'] = resize_array(obs['robot_head_rgb'], (128,128))
        
        # print('after reset:',env.env._env._sim.get_state()["robot_state"])
        self._obs_batching_cache = ObservationBatchingCache()
        batch = batch_obs(
            [obs], device=self.device, cache=self._obs_batching_cache
        )
        ...
        while len(all_episode_stats) < num_eval_episodes:
            ...
            if done:
                ...
                self.state_info = env_state_info
                # self.images = get_env_images(env, self.count_steps)
                # print('robot pos:', self.state_info['robot_state']['T'])
                # print('end nav goal:',env.env._env._task.nav_goals)
        ...
        # print('end',env.env._env._sim.get_state()["robot_state"])
        # Logging metrics
        aggregated_stats = {
        ...
        if 'pick' in self.config.VIDEO_DIR:
            if aggregated_stats['gripper_status.pick_correct'] == 1 and aggregated_stats['gripper_status.drop'] == 0:
                self.signal ='success'
        if 'nav' in self.config.VIDEO_DIR:
            if aggregated_stats['geo_dist_to_nav_goal'] < 0.65 and aggregated_stats['robot_force'] < 5.0:
                self.signal ='success'
        if 'place' in self.config.VIDEO_DIR:
            if aggregated_stats["obj_to_goal_dist"] < 0.5 and aggregated_stats['gripper_status.drop'] == 1:
                self.signal ='success'
